<?php
  $url = "/wiki/Categor%C3%ADa:ES:Adverbios";
  init($url);
  $counter = 1;
  ...
  function init($url){
    ...
    writeInfo($text,'<li><a href=',"adverbios.txt");
  ...
  function getUrlNext($text){
    foreach(explode("\n",$text) as $lines){
      $line = strstr($lines,'(<a href="/w/index.php?title=Categor%C3%ADa:ES:Adverbios&amp;pagefrom=');
      if($line){
        $parts = explode(" ",$line);
