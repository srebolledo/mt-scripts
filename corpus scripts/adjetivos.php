<?php
  $url = "/wiki/Categor%C3%ADa:ES:Adjetivos";
  init($url);
  $counter = 1;
  ...
  function init($url){
    ...
    writeInfo($text,'<li><a href=',"adjetivos.txt");
  ...
  function getUrlNext($text){
    foreach(explode("\n",$text) as $lines){
      $line = strstr($lines,'(<a href="/w/index.php?title=Categor%C3%ADa:ES:Adjetivos&amp;pagefrom=');
      if($line){
        $parts = explode(" ",$line);
